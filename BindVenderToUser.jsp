							...
							<table class="table table-bordered table-hover table-condensed" style="overflow-y:auto; width:100%;height:100px;border-spacing: 0px;">
								...
								<tbody role="alert" aria-live="polite" aria-relevant="all">
									<%
										...
										if(lst!=null)
										{
											List<clsGroupBean> group_lst=clsGroupBean.getGroupLst();
									...
									<tr class="odd" id="BMS<%=venderid%>">
										...
										<select id="userlist<%=venderid %>">
											...
											{
												out.print("<option "+ ((ubobj.getId()==obj.getAdminId())?"selected='selected'":"" +" value=\"")+ ubobj.getId() +"\">"+ ubobj.getAdminusername()+"-"+ubobj.getAdminname()+"</option>");
											}
											%>
										</select>
										...
										<td><%=obj.getAutoTransferRation() %></td>
										<td><%
											for(clsGroupBean gobj:group_lst)
											{
												if(gobj.getId()==obj.getGroupid())
												{
													out.print("["+gobj.getId()+"]");
													out.print(gobj.getGroupname());
													break;
												}
											}
										%></td>
